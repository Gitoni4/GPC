...
void drawHexagon(float colorR, float colorG, float colorB, float radius)
{
    glColor3f(colorR,colorG,colorB);
    glBegin(GL_POLYGON);
    float x, y;
    for (float degrees = 0; degrees < 360; degrees += 60)
    {
        x = cosf(degreeToRadian(degrees)) * radius;
        y = sinf(degreeToRadian(degrees)) * radius;
        glVertex2f(x, y);
    }
    glEnd();
...
void Display8() {
    ...
    drawHexagon(0.2, 0.15, 0.88, 0.6);
    drawHexagon(1.0, 0.1, 0.1, 0.45);
    drawHexagon(1.0, 1.0, 1.0, 0.40);
